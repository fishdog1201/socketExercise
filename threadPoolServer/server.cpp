...
void acceptConn(int lfd)
{
    ...
    while (1) {
        ...
        ThreadPoolSingleton& pool = ThreadPoolSingleton::getInstance();
        pool.enqueue(workFunc, cfd);
    }
...
int main()
{
    ...
    ThreadPoolSingleton& pool = ThreadPoolSingleton::getInstance(4);
    pool.enqueue(acceptConn, lfd);
